...
GLMatrixStack modeViewMatri;
GLMatrixStack projectionMatri;
GLFrame camerFrame;
GLFrame objectFrame;
GLFrustum viewFrutum;

GLBatch pyraminBatch;

//纹理变量 一般使用无符号整型
GLuint textureID;

GLGeometryTransform transformPipeline;
M3DMatrix44d shadowMatrix;
void RenderScene(void){
    ...
    modeViewMatri.MultMatrix(mCamera);
    ...
    modeViewMatri.MultMatrix(mObjectFrame);
...
void SepecialKeys(int key, int y, int z){
    if(key == GLUT_KEY_UP)
...
void ChangeSize(int w, int h){
    //1.设置视口
       ...
       //2.创建投影矩阵
       viewFrutum.SetPerspective(35.0f, float(w) / float(h), 1.0f, 500.0f);
     
       //viewFrustum.GetProjectionMatrix()  获取viewFrustum投影矩阵
       //并将其加载到投影矩阵堆栈上
       projectionMatri.LoadMatrix(viewFrutum.GetProjectionMatrix());
       
       //3.设置变换管道以使用两个矩阵堆栈（变换矩阵modelViewMatrix ，投影矩阵projectionMatrix）
       //初始化GLGeometryTransform 的实例transformPipeline.通过将它的内部指针设置为模型视图矩阵堆栈 和 投影矩阵堆栈实例，来完成初始化
       //当然这个操作也可以在SetupRC 函数中完成，但是在窗口大小改变时或者窗口创建时设置它们并没有坏处。而且这样可以一次性完成矩阵和管线的设置。
       transformPipeline.SetMatrixStacks(modeViewMatri, projectionMatri);
}
...
bool LoadTGATexture(const char *szFileName, GLenum minFilter, GLenum magFilter, GLenum wrapMode){
    ...
    int nWidth, nHight, nComponents;
    
    GLenum eFormat;
    
    // 读取纹理的像素
    pBits = gltReadTGABits(szFileName, &nWidth, &nHight, &nComponents, &eFormat);
    ...
    //参数8：指向纹理图像数据的指针
    glTexImage2D(GL_TEXTURE_2D, 0, nComponents, nWidth, nHight, 0, eFormat, GL_UNSIGNED_BYTE, pBits);
...
void SetupRC(){
    ...
    //创建金字塔
    MakePyramid(pyraminBatch);
    
    camerFrame.MoveForward(-10);
}
void ShutdownRC(){
